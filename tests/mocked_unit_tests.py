...
import app
from app import get_request_sid
...
from app import ALL_IDS_KEY, ALL_NAMES_KEY, ALL_IMAGES_KEY, ALL_CATEGORIES_KEY, ALL_USER_PRIMARY_IDS_KEY, ALL_DESCRIPTIONS_KEY, ALL_PROGRESS_KEY, ALL_POST_TEXTS_KEY
...
class SocketNewsFeedTestCase(unittest.TestCase):
    ...
    def test_on_new_google_user(self, mocked_db, mocked_server_socket):
        ...
        global mocked_user_id
        
        d_user = {  'email': 'test@njit.edu', 
                    'username': 'test username', 
                    'image': 'test img', 
                    'is_signed_in': 'Null', 
                    'id_token': 'test id token'}
                    
        expected_user_model = models.Users(d_user['email'],
                                            d_user['username'],
                                            d_user['image'],
                                            "Null",
                                            d_user['id_token'])
        ...
        mocked_server_socket.emit.assert_called_once_with('google info received', mock_personal_profile)
    ...
    def test_emit_newsfeed(self, mocked_db, mocked_server_socket):
        mock_all_ids = [
            mock_DB_id.id
            for mock_DB_id in mocked_db.session.query(models.Users).all()
        ]
        mock_all_names = [
            mock_DB_name.name
        ...
        for mocked_db_users, mocked_db_goals in mocked_db.session.query(models.Users, models.Goals).filter(models.Users.id == models.Goals.user_id).order_by(models.Goals.date).all():
        ...
        res = app.emit_newsfeed(EMIT_EXERCISE_NEWSFEED_CHANNEL)
